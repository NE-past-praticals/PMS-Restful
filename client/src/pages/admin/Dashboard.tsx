...
export function Dashboard() {
  ...
  const { data: ordersData, isLoading: ordersLoading } = useSlotOrders({ limit: 100 });
  ...
  const [slotSizeData, setSlotSizeData] = useState<{ name: string; count: number }[]>([]);
  
  // Chart data for slot statuses
  const [slotStatusData, setSlotStatusData] = useState<{ name: SlotStatus; count: number }[]>([]);

  useEffect(() => {
    if (slotsData?.data) {
      const available = slotsData.data.filter(slot => slot.slotStatus === 'AVAILABLE').length;
      const occupied = slotsData.data.filter(slot => slot.slotStatus === 'OCCUPIED').length;
      const maintenance = slotsData.data.filter(slot => slot.slotStatus === 'MAINTENANCE').length;
      
      // Count slots by size
      const sizeCount: Record<SlotSize, number> = {
        SMALL: 0,
        MEDIUM: 0,
        LARGE: 0,
      };
      ...
      const statusData = [
        { name: 'AVAILABLE' as SlotStatus, count: available },
        { name: 'OCCUPIED' as SlotStatus, count: occupied },
        { name: 'MAINTENANCE' as SlotStatus, count: maintenance },
      ];
  ...
  useEffect(() => {
    if (ordersData?.data) {
      const active = ordersData.data.filter(order => 
        order.slotOrderStatus === 'PENDING' || order.slotOrderStatus === 'APPROVED'
      ).length;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {Math.round((stats.availableSlots / stats.totalSlots) * 100) || 0}% of total capacity
                </p>
              </CardContent>
            </Card>
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={slotSizeData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="count" name="Number of Slots" fill="#3B82F6" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={slotStatusData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="count" name="Number of Slots" fill="#10B981" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
