...
import { Slot, SlotSize, SlotStatus } from '@/api/slotApi';
...
export function SlotsList() {
  ...
  const [selectedSlotStatus, setSelectedSlotStatus] = useState<SlotStatus | null>(null);
  ...
  const columns: Column<Slot>[] = [
    ...
    },
    {
      ...
      cell: (slot) => format(new Date(slot.createdAt || ''), 'MMM dd, yyyy HH:mm'),
      sortable: true,
    },
    {
      header: 'Actions',
  ...
  const handleSlotSizeChange = (value: string) => {
    setSelectedSlotSize(value === "all" ? null : value as SlotSize);
  };

  const handleSlotStatusChange = (value: string) => {
    setSelectedSlotStatus(value === "all" ? null : value as SlotStatus);
  };
    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap justify-end gap-4 items-center">
        <Dialog open={multipleModalOpen} onOpenChange={setMultipleModalOpen}>
          ...
          <DialogContent>
            <BulkCreateSlots open={multipleModalOpen} openChange={setMultipleModalOpen}/>
          </DialogContent>
        </Dialog>
        
        <Select value={selectedSlotSize ? selectedSlotSize : "all"} onValueChange={handleSlotSizeChange}>
        ...
        <Select value={selectedSlotStatus ? selectedSlotStatus : "all"} onValueChange={handleSlotStatusChange}>
          <SelectTrigger className="w-[180px]">
